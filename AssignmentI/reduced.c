...
int main(){
    ...
    for(int step=1;step<=n_steps;step++){
        ...
        for (int q=0;q<N-1;q++){
            for (int k=q+1;k<N-1;k++) {
                ...
                force_qk[0] = G*mass[q]*mass[k]/dist_cubed * x_diff;
                force_qk[1] = G*mass[q]*mass[k]/dist_cubed * y_diff;
    ...
    t = mysecond() - ts;
    printf("Time for Reduced Algorithom of Nbody problem when N=,%d, time : T = %f /s\n", N, t);
