...
int DFT(int idft, double *xr, double *xi, double *Xr_o, double *Xi_o, int N,double *real_sum_n,double *img_sum_n);
...
int main(int argc, char *argv[]) {
  ...
  double real_sum_n[MAX_THREADS];
  double img_sum_n[MAX_THREADS];
  // start timer
  ...
  // DFT
  int idft = 1;
  DFT(idft, xr, xi, Xr_o, Xi_o, N,real_sum_n,img_sum_n);
  // IDFT
  idft = -1;
  DFT(idft, Xr_o, Xi_o, xr_check, xi_check, N,real_sum_n,img_sum_n);
  ...
  //  for(j=1;j<=MAX_THREADS;j++)
    //{
      ...
      #pragma omp parallel 
      {
        ...
        real_sum_n[id]=0;
        img_sum_n[id]=0;
        for (i=id ; i< N; i+=MAX_THREADS) 
        {
          real_sum_n[id]+=xr[i] * cos(i * k* PI2 / N) + idft * xi[i] * sin(i * k * PI2 / N);
          img_sum_n[id]+=-idft * xr[i] * sin(i * k * PI2 / N) + xi[i] * cos(i * k * PI2 / N);
        } 
      }
